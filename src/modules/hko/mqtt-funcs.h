...
inline void mosq_on_connect(struct mosquitto *mosq, void *obj, int reason_code) {
  (void)mosq;
  (void)obj;
  syslog(LOG_INFO, "mosq_on_connect: %s", mosquitto_connack_string(reason_code));
}

inline void mosq_on_disconnect(struct mosquitto *mosq, void *obj, int reason_code) {  
  (void)mosq;
  (void)obj;
  syslog(LOG_INFO, "mosq_on_disconnect(): %s", mosquitto_connack_string(reason_code));
...
inline struct mosquitto *initMosquitto(const char* host, const char* username, const char* password) {
  int rc;
  ...
  if ((rc = mosquitto_tls_set(mosq, "/etc/ssl/certs/ca-certificates.crt", NULL,
                              NULL, NULL, NULL)) != MOSQ_ERR_SUCCESS) {
