...
struct CallbackContext callback_init(const json_object *config) {
  ...
  conn.display_digit_count = json_object_get_int(root_7sd_display_digit_count);
  ...
  strncpy(conn.gpiochip_path, json_object_get_string(root_7sd_gpiochip_path),
          PATH_MAX);
  if (conn.display_digit_count == 0 || conn.data_pin_num == 0 ||
      conn.clock_pin_num == 0 || conn.latch_pin_num == 0 ||
      conn.chain_num == 0 || strlen(conn.gpiochip_path) == 0) {
    SYSLOG_ERR("Some required values are not provided");
  }

  syslog(LOG_INFO, "display_digit_count: %zu", conn.display_digit_count);
...
int callback(const struct SensorReadings pl, struct CallbackContext *ctx) {
  ...
  iotctrl_7seg_disp_update_2floats(*h, (float)pl.temp_celsius,
